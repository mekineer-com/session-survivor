...
from chat_grok_session import read_rows
    ...
    for row in updates:
        ...
        if kind == 'user_message_chunk':
            ...
            if not isinstance(index, int) or index < first_prompt:
                ...
                continue
            current = index
            rows.append((row['timestamp'], index, 'Marcos', event['content']['text']))
        elif kind == 'agent_message_chunk' and current is not None:
            text = event.get('content', {}).get('text')
            if isinstance(text, str) and text:
                rows.append((row['timestamp'], current, 'Rook', text))
    return rows
...
def export(session, output):
    ...
    rows = collect(read_rows(session / 'updates.jsonl'), min(native))
    if not rows:
        raise ValueError('No Grok dialogue found')
    output.mkdir(parents=True)
    days = {}
    for stamp, index, role, text in rows:
        dt = datetime.fromtimestamp(stamp, timezone.utc)
        days.setdefault(dt.date().isoformat(), []).append((dt, index, role, text))
    index_lines = ['# Grok Summary Source', '', f'- Source: `{session}`',
                   f'- First available prompt: `{min(native)}`', '', '## Read Order', '']
