...
python3 "$CHAT_TOOL" "$SOURCE" --output-root "$OUTROOT/chat-resume-boundary-safe" --show-summary > "$OUTROOT/chat-resume-boundary-safe-run.json"
CHAT_REPORT="$(find "$OUTROOT/chat-resume-boundary-safe/reports" -type f -name '*.json' | head -n 1)"

printf 'source=%s\n' "$SOURCE"
printf 'outroot=%s\n' "$OUTROOT"
printf 'safe_report=%s\n' "$SAFE_REPORT"
printf 'resume_report=%s\n' "$RESUME_REPORT"
printf 'chat_resume_boundary_safe_report=%s\n' "$CHAT_REPORT"
