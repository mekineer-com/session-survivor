...
python3 "$CHAT_TOOL" "$SOURCE" --output-root "$OUTROOT/chat-resume-hybrid-safe-tail" --show-summary > "$OUTROOT/chat-resume-hybrid-safe-tail-run.json"
CHAT_REPORT="$(find "$OUTROOT/chat-resume-hybrid-safe-tail/reports" -type f -name '*.json' | head -n 1)"

printf 'source=%s\n' "$SOURCE"
printf 'outroot=%s\n' "$OUTROOT"
printf 'safe_report=%s\n' "$SAFE_REPORT"
printf 'resume_report=%s\n' "$RESUME_REPORT"
printf 'chat_resume_hybrid_safe_tail_report=%s\n' "$CHAT_REPORT"
